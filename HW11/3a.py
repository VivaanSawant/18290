...
def function_H(x, x_siz, theta):
    x = x.reshape(x_siz)
    y0 = radon(x, theta=theta)
    return y0

def function_H_transpose(y, y_siz, theta):
    y = y.reshape(y_siz)
    z0 = iradon(y, theta=theta, filter_name=None)
    return z0
...
def cgs(A, b, x0, max_iter=20, tol=1e-6):
    ...
    for i in range(max_iter):
        ...
        print("Iteration", i, "Residual", np.sqrt(rsnew), flush=True)

        if np.sqrt(rsnew) < tol:
            print("Converged at iteration", i, flush=True)
            break
...
print("Part A: Reconstructed image x_star")
print(x_star)
